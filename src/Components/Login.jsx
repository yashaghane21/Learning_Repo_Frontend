...
export default function SignUp() {
  ...
  const handlesubmit = async (e) => {
    ...
    try {
      const { data } = await axios.post(`${server}/api/v3/login`, {
        email: email,
        password: password,
      });
      if (data.success) {
      ...
      <div className="w-100 mt-0 justify-center pt-16 px-5 bg-gray-200 lg:px-72 md:pb-52 max-md:pb-16">
        <div className="w-[100%] bg-white flex md:flex-row max-md:flex-col-reverse p-3 max-md:pb-20  shadow-xl rounded-md">
          <div className="w-[60%] max-md:w-[100%] h-auto text-center text-2xl font-semibold p-2 ">
            <h1 className="font underline underline-offset-2 max-md:mt-5">
              Login
            </h1>
            <div className="mt-7 ">
              <form onSubmit={handlesubmit}>
                ...
                <p className="text-sm md:mr-20 max-sm:mr-12 sm:mr-16 font-normal mt-4 float-right  ">
                  {" "}
                  <Link className="text-green-600" to="/Forgot ">
                    Forgot Password ?
                  </Link>
                </p>
                <button className="w-[80%] mt-6 p-2 text-lg bg-yellow-400 hover:bg-yellow-300 rounded-lg">
                  Login
                </button>
              </form>

              <p className="text-sm font-normal mt-12 max-md:mt-8">
          ...
          <div className="w-[40%] max-md:w-[100%] h-auto ">
            <img
              src={img1}
              className="h-96 w-100 max-md:w-[100%] max-md:h-52 rounded-md"
            />
          </div>
        </div>
      </div>
